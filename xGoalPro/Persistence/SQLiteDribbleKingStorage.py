...
class SQLiteDribbleKingStorage(IRiskGameStorage):
    """SQLite-based storage for the Dribble King game."""
    ...
    async def insert_user_score(self, user_id: int, score: int, high_score: int) -> None:
        ...
        await self.__conn.execute(
            "INSERT OR IGNORE INTO dribbleScore (user_id, score, high_score) VALUES (?, ?, ?)",
    ...
    async def update_user_score(self, user_id: int, score: int, high_score: int) -> None:
        ...
        await self.__conn.execute(
            "UPDATE dribbleScore SET score = ?, high_score = ? WHERE user_id = ?",
    ...
    async def get_user_score(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        async with self.__conn.execute(
                "SELECT score, high_score FROM dribbleScore WHERE user_id = ?",
                (user_id,)
        ) as cursor:
    ...
    async def user_exists(self, user_id: int) -> bool:
        ...
        async with self.__conn.execute("SELECT id FROM dribbleScore WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
