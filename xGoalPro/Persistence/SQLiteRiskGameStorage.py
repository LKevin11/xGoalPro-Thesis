...
class SQLiteRiskGameStorage(IRiskGameStorage):
    """SQLite-based storage for the Risk game."""
    ...
    async def insert_user_score(self, user_id: int, score: int, high_score: int) -> None:
        ...
        await self.__conn.execute(
            "INSERT OR IGNORE INTO riskScore (user_id, bank, high_score) VALUES (?, ?, ?)",
    ...
    async def update_user_score(self, user_id: int, score: int, high_score: int) -> None:
        ...
        await self.__conn.execute(
            "UPDATE riskScore SET bank = ?, high_score = ? WHERE user_id = ?",
            (score, high_score, user_id)
        )
        await self.__conn.commit()

    async def get_user_score(self, user_id: int) -> Optional[Dict[str, Any]]:
        """Retrieves the bank and high score for the specified user."""
        async with self.__conn.execute(
                "SELECT bank, high_score FROM riskScore WHERE user_id = ?",
                (user_id,)
        ) as cursor:
    ...
    async def user_exists(self, user_id: int) -> bool:
        ...
        async with self.__conn.execute("SELECT id FROM riskScore WHERE user_id = ?", (user_id,)) as cursor:
            row = await cursor.fetchone()
